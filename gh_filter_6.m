...
measurements = gen_data(10, 0, 20, 0, 9);
data = g_h_filter(measurements, 10, 0, 1, .2, 0.02);
...
function measurements = gen_data(x0, dx, count, noise_factor, accel)
    measurements = zeros(1,count);
    for i = 1:count
        measurements(i) = x0 + accel * (i^2) / 2 + dx*i + randn()*noise_factor;
    end
end
